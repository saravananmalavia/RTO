...
public class TempDL
{
	...
	public static ArrayList<TempDTO> getTPAllotStatus()
	{
		...
		catch(Exception e)
		{
			...
			System.out.println("**Error**: TempDL:getTPAllotStatus():"+e.getMessage());
		}
		return temps;
	}
	...
	public static int getCountTPAlloted()
	{
		...
		try
		{
			String sql="select count(*) as total1 from tb_tp_alloted where status=1";
			con=DBHelper.getConnection();
			statement=con.createStatement();
			resultSet=statement.executeQuery(sql);
			if(resultSet.next())
			{
				count_tp_alloted=resultSet.getInt("total1");
			}
			con.close();
		}
		catch(Exception e)
